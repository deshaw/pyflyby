...
def main() -> None:
    ...
    if options.include:
        regexps = [
            re.escape(prefix) if prefix.endswith(".") else re.escape(prefix) + "([.]|$)"
            for prefix in options.include
        ]
        regexp = re.compile("|".join(regexps))
        match = lambda imp: regexp.match(imp.fullname)
        importset = ImportSet([imp for imp in importset if match(imp)])  # type: ignore [no-untyped-call]
    if options.ignore_known:
        db = ImportDB.get_default(".")
        importset = importset.without_imports(db.known_imports)
    sys.stdout.write(
        importset.pretty_print(allow_conflicts=True, params=options.params)
    )
