...
def symbol_needs_import(fullname, namespaces, using_scope_name=None):
    """
    ...
    :param using_scope_name:
      Optional scope name where this symbol is being used (for Phase 3 tracking).
    :type namespaces:
      ``list`` of ``dict``
    :param namespaces:
    ...
    for ns_idx, ns in reversed(list(enumerate(namespaces))):
        ...
        for partial_name in partial_names:
            ...
            if isinstance(var, _UseChecker):
                var.used = True
                # For Phase 3: also track which scope is using this import
                if hasattr(var, "mark_used_in_scope"):
...
class _UseChecker:
    ...
    used_in_scopes: List[Optional[str]] = field(default_factory=list)  # Track WHERE it's used (for Phase 3)
    ...
    def __init__(
        ...
        self.used_in_scopes = []  # List of scope names where this import is used
        logger.debug("Create _UseChecker : %r", self)

    def mark_used_in_scope(self, using_scope_name: Optional[str]):
        """Mark this import as used in a specific scope (for Phase 3 cross-scope analysis)."""
        self.used = True
...
class _MissingImportFinder:
    ...
    unused_imports: Optional[List[Union[Tuple[int, str], Tuple[int, str, str]]]]
            ...
            if literal_brace_identifiers:
                for ident in literal_brace_identifiers:
                    try:
                        ...
                        continue
                    # Pass current scope for Phase 3 tracking (usually None/global here)
                    current_scope = (
        ...
        try:
            ...
            for name, use_checker in new_scopestack[-1].items():
                if use_checker and use_checker.used == False and check_unused_imports:
                    ...
                    # Include scope_name if available (for local imports in functions/classes)
                    if self.unused_imports is not None:
                        if use_checker.scope_name:
                            ...
                            self.unused_imports.append(
                                (use_checker.lineno, use_checker.source)
                            )
    ...
    def _visit_Load_defered_global(self, fullname:str):
        ...
        # Store scope name for Phase 3 tracking
    ...
    def _check_load(self, fullname, scopestack, lineno):
        ...
        # Pass current scope for Phase 3 tracking
        current_scope = self._scope_name_stack[-1] if self._scope_name_stack else None
        if (
    ...
    def analyze_cross_scope_redundancies(self):
        """
        ...
        all_imports = {}  # name -> list of _UseChecker objects
        ...
        for import_name, checkers in all_imports.items():
            ...
            for global_imp in global_imports:
                ...
                scopes_with_local = set(c.scope_name for c in local_imports if c.used)
                ...
                if scopes_with_local and global_imp.used_in_scopes:
                    ...
                    global_used_scopes = set(global_imp.used_in_scopes)
                    # Remove None (global scope usage) from the set for this check
                    global_used_in_functions = global_used_scopes - {None}
                    ...
                    if global_used_in_functions and global_used_in_functions.issubset(
                        ...
                        logger.info(
                            "Phase 3: Global import %s is redundant (shadowed by local imports in %s)",
                            global_imp.name,
                            scopes_with_local,
                        )
